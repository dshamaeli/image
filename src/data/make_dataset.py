...
def build_HDF5(jpeg_dir, nb_channels, size=256):
    ...
    with h5py.File(hdf5_file, "w") as hfw:
        ...
        for dset_type in ["train", "test", "val"]:
            ...
            list_img = [img for img in Path(
                jpeg_dir).glob('%s/*.jpg' % dset_type)]
            ...
            for chunk_idx in tqdm(arr_chunks):
                ...
                output = parmap.map(format_image, list_img_path,
                                    size, nb_channels, pm_parallel=False)
                ...
                # Resize HDF5 dataset
                data_full.resize(
                    data_full.shape[0] + arr_img_full.shape[0], axis=0)
                data_sketch.resize(
                    data_sketch.shape[0] + arr_img_sketch.shape[0], axis=0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('nb_channels', type=int,
