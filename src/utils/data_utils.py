...
def get_nb_patch(img_dim, patch_size, image_data_format):
    ...
    assert image_data_format in ["channels_first",
                                 "channels_last"], "Bad image_data_format"
    ...
    if image_data_format == "channels_first":
        ...
        nb_patch = (img_dim[1] // patch_size[0]) * \
            (img_dim[2] // patch_size[1])
        ...
        nb_patch = (img_dim[0] // patch_size[0]) * \
...
def plot_generated_batch(X_full, X_sketch, generator_model, batch_size, image_data_format, suffix):
    ...
    if image_data_format == "channels_last":
        ...
        for i in range(int(X.shape[0] // 4)):
            Xr = np.concatenate([X[k]
                                 for k in range(4 * i, 4 * (i + 1))], axis=1)
            list_rows.append(Xr)
    ...
    if image_data_format == "channels_first":
        ...
        for i in range(int(X.shape[0] // 4)):
            Xr = np.concatenate([X[k]
                                 for k in range(4 * i, 4 * (i + 1))], axis=2)
            list_rows.append(Xr)
